...
当前 GitHub 仓库以 `main` 分支为准，采用前后端同仓库管理。仓库根目录只保留工程入口、文档和配置，业务代码按职责分区：

- 后端项目位于 `backend/`，入口为 `backend/pom.xml` 和 `backend/src/`
- 前端项目位于 `frontend/`
- 数据库脚本位于 `database/`
- 部署和排查文档位于 `docs/`

如果本地还存在独立目录 `D:\java-project\campus-service-web`，它只是历史副本或临时开发目录，不作为当前仓库的维护对象。后续开发、提交和推送请统一使用本仓库内的 `frontend/`。
...
`database/campus.sql` 是完整建库脚本，会删除并重建 `campus_service` 数据库，只适合本地开发、测试环境或确认允许清空数据的场景。
...
后端默认读取 `backend/src/main/resources/application.yml`，常用配置可以用环境变量覆盖。
...
```text
backend/target/campus-service-1.0-SNAPSHOT.jar
```
...
3. 使用 `database/maintenance.sql` 中的管理员维护 SQL，将该用户设置为管理员。
...
- `database/campus.sql`：完整建库脚本，会删除并重建数据库
- `database/maintenance.sql`：常用维护 SQL，例如管理员授权、分类维护、数据检查
- `database/fix-activity-sub-category.sql`：活动通知子分类相关修复脚本
- `database/optimize-private-chat.sql`：私聊相关优化脚本
- `database/REBUILD.md`：测试环境重建数据库说明
